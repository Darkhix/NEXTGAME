...
class BattleHistoryForm(ctk.CTkToplevel):
    def __init__(self, master, callback, battle_data=None, record_index=None):
        ...
        self.title("Editar Registro" if is_editing else "Añadir Registro")
    ...
    def save(self):
        ...
        if self.record_index is not None:
            ...
            history.insert(0, new_record)
        save_json_data("battle_history.json", history)
        messagebox.showinfo("Éxito", "El historial ha sido actualizado.")
        self.callback()
        self.on_closing()
...
class Game:
    ...
    def load_general_assets(self):
        ...
        for key, data in config.MAPS.items():
            try:
                ...
                placeholder = pygame.Surface((220, 165))
                placeholder.fill(config.GRAY)
    ...
    def handle_events(self):
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                elif self.game_state == 'map_select': self.handle_map_select_keys(event.key)
    ...
    def handle_map_select_keys(self, key):
        ...
        if key == pygame.K_RIGHT:
            self.map_select_idx = (self.map_select_idx + 1) % num_maps
            ...
            try:
                ...
                print(f"ADVERTENCIA: No se pudo cargar el fondo del mapa '{self.selected_map_key}'. Error: {e}")
                default_bg_path = getattr(config, 'BG_IMG_PATH', 'assets/images/background/background.jpg')
                self.bg_image = pygame.image.load(default_bg_path).convert_alpha()
            self.reset_round()
    ...
    def handle_crud_keys(self, key):
        ...
        if key == pygame.K_UP: self.crud_char_idx = (self.crud_char_idx - 1) % len(char_names) if char_names else 0
            ...
            if selected_option == 'Añadir Personaje':
                root = ctk.CTk(); root.withdraw(); CharacterForm(root, self.refresh_character_data); root.mainloop()
            elif selected_option == 'Ver Datos':
                char_data = self.all_characters_data.get(selected_char_name)
                root = ctk.CTk(); root.withdraw(); CharacterForm(root, self.refresh_character_data, char_data, selected_char_name, read_only=True); root.mainloop()
            elif selected_option == 'Editar Estadísticas':
                root = ctk.CTk(); root.withdraw(); StatEditForm(root, self.refresh_character_data, selected_char_name); root.mainloop()
    ...
    def handle_battle_history_keys(self, event):
        ...
        if key == pygame.K_ESCAPE: self.game_state = 'menu'
            ...
            if self.battle_history: self.history_selected_idx = max(0, self.history_selected_idx - 1)
        elif key == pygame.K_DOWN:
            if self.battle_history: self.history_selected_idx = min(len(self.battle_history) - 1, self.history_selected_idx + 1)
        elif key == pygame.K_a:
            root = ctk.CTk()
            root.withdraw()
    ...
    def draw_scenes(self):
        if self.game_state == 'menu': ui.draw_menu(self.screen, self.menu_font, config.MENU_ITEMS, self.menu_idx)
            ...
            ui.draw_map_select(self.screen, self.title_font, self.menu_font, config.MAPS, self.map_thumbnails, self.map_select_idx)
