...
def draw_map_select(surface, title_font, text_font, maps_data, thumbnails, selected_idx):
    surface.fill(config.BLACK)
    title_text = title_font.render("Selecciona un Mapa", True, config.WHITE)
    surface.blit(title_text, (surface.get_width() // 2 - title_text.get_width() // 2, 80))
    map_keys = list(maps_data.keys())
    num_maps = len(map_keys)
    total_width = num_maps * 220 + (num_maps - 1) * 30
    start_x = (surface.get_width() - total_width) // 2
    base_y = surface.get_height() // 2 - 100
    for i, key in enumerate(map_keys):
        map_x = start_x + i * 250
        thumbnail = thumbnails.get(key)
        if thumbnail:
            scaled_thumb = pygame.transform.scale(thumbnail, (220, 165))
            surface.blit(scaled_thumb, (map_x, base_y))
        color = config.HIGHLIGHT if i == selected_idx else config.GRAY
        map_name_text = text_font.render(key, True, color)
        surface.blit(map_name_text, (map_x + (220 - map_name_text.get_width()) // 2, base_y + 180))
        if i == selected_idx:
            pygame.draw.rect(surface, config.HIGHLIGHT, (map_x - 5, base_y - 5, 230, 175), 4, border_radius=5)
    help_text = text_font.render("Usa las flechas ← → para navegar y ENTER para seleccionar", True, config.WHITE)
    surface.blit(help_text, (surface.get_width() // 2 - help_text.get_width() // 2, surface.get_height() - 80))